      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <div className="space-y-4">
            {(
              Object.keys(FEATURES_DATA) as Array<keyof typeof FEATURES_DATA>
            ).map((key) => (
              <button
                key={key}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  {/* Note for self: In Tailwind, dynamic template literals like `bg-${color}-500` can sometimes be ignored. 
                    If the dots don't show colors in prod, replace this with a standard mapping object or safe-list the colors. */}
                  {activeTab === key && (
                    <span
                      className={`h-2 w-2 rounded-full bg-${FEATURES_DATA[key].color}-500 shadow-[0_0_10px_currentColor]`}
                    ></span>
                  )}
                </div>
        ...
        <div className="h-[500px] w-full rounded-3xl border border-white/10 bg-black/50 backdrop-blur-xl shadow-2xl overflow-hidden relative">
          ...
          <div
            className={`${activeTab !== "scan" ? "pt-10" : ""} h-full w-full`}
          >
            {current.preview}
          </div>
        </div>
